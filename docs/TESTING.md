...
| `e2e7-progress.mjs` | Playwright | Three things. **The progress estimate**: labelled as an estimate, never reaches 100% early, never moves backwards, and is ahead of solved-blocks mid-transfer. **The decode rate**: codes are fed in bursts of nine sharing a timestamp, as a real grid frame arrives, and the reported rate must not exceed what was actually fed — the regression test for a field reading of 141.5/s against a physical ceiling of 135. **The size ladder**: quiet under 64 MB, an informed confirm up to 512 MB, refusal above. **Auto-save**: a completed transfer fires a download with the sender's filename, without a click. |
| `bench.js` | Node | **Not pass/fail.** Encoder frames/s, encoder memory, decoder full-run time at large K. The before/after evidence for every efficiency change (`DECISIONS.md` §21); run it against a proposed core change before believing the change helps. |
